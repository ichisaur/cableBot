...
volatile char serialCom[10]={0};
volatile char RsInfo[20]={0};
volatile int RsInfolength = 0;
...
UART0IntHandler(void)
{
   ...
   // TODO: processing the hwType char doesnt work. May not needed, hard program in based on hwID
    if (myInput[0]=='5'){
        sscanf(myInput, "%1i %1c %s %i", &hwID, &hwType, &param3, &lengthS);
    }
    else{
...
//    int length = sprintf(buffer, "%d %c %d %d \r\n", hwID, hwType, param1, param2);
    ...
    // Echo value received back to PySerial
    if(hwID != 5){
        length = sprintf(buffer, "%d %c %d %d \r\n", hwID, hwType, param1, param2);
    }
    else{
        length = sprintf(buffer, "%d %c %s %d \r\n", hwID, hwType, serialCom, lengthS);
        UART1Send( (uint8_t *)serialCom,lengthS);
        UART1Send( (uint8_t *) "\r\n", 2);
    }
...
UART1IntHandler(void)
{
    ...
    RsInfolength=UART1Read(&RsInfo);

//    UART0Send( (uint8_t *) RsInfo, RsInfolength);
//    UART0Send( (uint8_t *) "\r\n", 2);
